...
namespace ECX.Website.API.Controllers
{
    ...
    public class ExternalLinkController : ControllerBase
    {
        ...
        public ExternalLinkController(IMediator mediator)
        {
            _mediator = mediator;
        }

        // GET: api/<ExternalLinkController>
        [HttpGet]
        public async Task<ActionResult<BaseCommonResponse>> Get()
        {
            var query = new GetExternalLinkListRequest();
        ...
        // GET api/<ExternalLinkController>/5
        [HttpGet("{id}")]
        public async Task<ActionResult<BaseCommonResponse>> Get(string id)
        {
            var query = new GetExternalLinkDetailRequest { Id = id };
            BaseCommonResponse response = await _mediator.Send(query);
            switch(response.Status){
                case "200" : return Ok(response);
                case "400" : return BadRequest(response);
        ...
        // POST api/<ExternalLinkController>
        [HttpPost]
        public async Task<ActionResult<BaseCommonResponse>> Post([FromForm] ExternalLinkFormDto data)
        {
            var command = new CreateExternalLinkCommand { ExternalLinkFormDto = data };
        ...
        // PUT api/<ExternalLinkController>/5
        [HttpPut]
        public async Task<ActionResult<BaseCommonResponse>> Put([FromForm] ExternalLinkFormDto data)
        {
            var command = new UpdateExternalLinkCommand { ExternalLinkFormDto = data};
            BaseCommonResponse response = await _mediator.Send(command);
            switch(response.Status){
                case "200" : return Ok(response);
                case "400" : return BadRequest(response);
        ...
        // DELETE api/<ExternalLinkController>/5
        [HttpDelete("{id}")]
