...
namespace ECX.Website.API.Controllers
{
    ...
    public class ApplicantController : ControllerBase
    {
        ...
        {
            var query = new GetApplicantListRequest();
        ...
        {
            var query = new GetApplicantDetailRequest { Id = id };
            BaseCommonResponse response = await _mediator.Send(query);
            switch(response.Status){
                case "200" : return Ok(response);
                case "400" : return BadRequest(response);
        ...
        [HttpPost]
        public async Task<ActionResult<BaseCommonResponse>> Post([FromForm] ApplicantFormDto data)
        {
            var command = new CreateApplicantCommand { ApplicantFormDto = data };
        ...
        [HttpPut]
        public async Task<ActionResult<BaseCommonResponse>> Put([FromForm] ApplicantFormDto data)
        {
            var command = new UpdateApplicantCommand { ApplicantFormDto = data};
            BaseCommonResponse response = await _mediator.Send(command);
            switch(response.Status){
                case "200" : return Ok(response);
                case "400" : return BadRequest(response);
        ...
        {
            var command = new DeleteApplicantCommand { Id = id };
            BaseCommonResponse response = await _mediator.Send(command);
            switch(response.Status){
                case "200" : return Ok(response);
                case "400" : return BadRequest(response);
